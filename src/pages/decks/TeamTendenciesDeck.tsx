...
const SETS: Record<string, { title: string; sub: string; defs: MDef[] }> = {
  ...
  sit: { title: 'Situational behavior', sub: 'third down, red zone, goal line — by team', defs: [
    ...
    { key: 'ypsit', label: 'Yds / Play', expr: 'round(sum(COALESCE(passing_yards,0)+COALESCE(rushing_yards,0))*1.0/count(*),2)', f: 'd2' },
  ] },
  expl: { title: 'Explosive plays & scoring', sub: 'by team', defs: [
    ...
    { key: 'fdpct', label: '1st Down %', expr: 'round(sum(first_down)*100.0/count(*),1)', f: 'pct' },
    { key: 'ypp2', label: 'Yds / Play', expr: 'round(sum(COALESCE(passing_yards,0)+COALESCE(rushing_yards,0))*1.0/count(*),2)', f: 'd2' },
  ] },
  eff: { title: 'Overall efficiency', sub: 'by team', defs: [
    { key: 'ypp2', label: 'Yds / Play', expr: 'round(sum(COALESCE(passing_yards,0)+COALESCE(rushing_yards,0))*1.0/count(*),2)', f: 'd2' }, { key: 'pypa', label: 'Pass Y/P', expr: 'round(sum(passing_yards) FILTER(WHERE pass_attempt=1)*1.0/nullif(count(*) FILTER(WHERE pass_attempt=1),0),2)', f: 'd2' },
    { key: 'rypa', label: 'Rush Y/P', expr: 'round(sum(rushing_yards) FILTER(WHERE rush_attempt=1)*1.0/nullif(count(*) FILTER(WHERE rush_attempt=1),0),2)', f: 'd2' }, { key: 'fdpct2', label: '1st Down %', expr: 'round(sum(first_down)*100.0/count(*),1)', f: 'pct' },
    { key: 'p_fdpct', label: 'Pass 1D%', expr: 'round(sum(first_down) FILTER(WHERE pass_attempt=1)*100.0/nullif(count(*) FILTER(WHERE pass_attempt=1),0),1)', f: 'pct' },
    { key: 'r_fdpct', label: 'Rush 1D%', expr: 'round(sum(first_down) FILTER(WHERE rush_attempt=1)*100.0/nullif(count(*) FILTER(WHERE rush_attempt=1),0),1)', f: 'pct' },
...
function DataTab() {
  ...
  const sql = `SELECT posteam tm, count(*) plays,
      ...
      round(avg(air_yards) FILTER(WHERE pass_attempt=1),1) adot, round(sum(COALESCE(passing_yards,0)+COALESCE(rushing_yards,0))*1.0/count(*),2) ypp,
      round(sum(passing_yards) FILTER(WHERE pass_attempt=1)*1.0/nullif(count(*) FILTER(WHERE pass_attempt=1),0),2) pypa,
      round(sum(rushing_yards) FILTER(WHERE rush_attempt=1)*1.0/nullif(count(*) FILTER(WHERE rush_attempt=1),0),2) rypa,
      sum(first_down)::int fd, round(sum(first_down)*100.0/count(*),1) fdpct,
    ...
    GROUP BY 1 HAVING count(*) >= ${minN(slicers)} ORDER BY ypp DESC`
  const q = useQuery<any>(sql, [sql])
  const cols: Column<any>[] = [
    ...
    { key: 'adot', label: 'aDOT', numeric: true, format: F.d1 }, { key: 'ypp', label: 'Y/P', numeric: true, format: F.d2 }, { key: 'pypa', label: 'PaY/A', numeric: true, format: F.d2 }, { key: 'rypa', label: 'RuY/A', numeric: true, format: F.d2 },
  ...
  return <Tile title="Every offense — packed tendencies" subtitle={`${sliceLabel(slicers)} · honors all rail filters · scroll horizontally`} span={12}>
    <QueryState q={q} height={460}>{rows => <DataTable rows={rows} columns={cols} defaultSort={{ key: 'ypp', dir: 'desc' }} tight zebra />}</QueryState>
  </Tile>
}
