...
const SETS: Record<string, { title: string; sub: string; base: string; defs: MDef[] }> = {
  overall: { title: 'Overall defense', sub: 'Directly measurable outcomes', base: '(pass_attempt=1 OR rush_attempt=1)', defs: [
  ...
  dir: { title: 'Pass defense by direction', sub: '1st downs allowed by side of field', base: 'pass_attempt=1 AND pass_location IS NOT NULL', defs: [
  ...
  run: { title: 'Run defense', sub: 'YPC & stuffs allowed, by defense', base: 'rush_attempt=1', defs: [
...
function DataTab() {
  ...
  const sql = `SELECT defteam tm, count(*) plays, round(sum(COALESCE(passing_yards,0)+COALESCE(rushing_yards,0))*1.0/count(*),2) ypp,
    ...
    GROUP BY 1 HAVING count(*) >= ${minN(slicers)} ORDER BY ypp ASC`
  const q = useQuery<any>(sql, [sql])
  const cols: Column<any>[] = [
    { key: 'tm', label: 'Def' }, { key: 'plays', label: 'Plays', numeric: true },
    { key: 'ypp', label: 'Y/P', numeric: true, format: F.d2 },
    { key: 'pypa', label: 'PaY/A', numeric: true, format: F.d2 },
    { key: 'rypa', label: 'RuY/A', numeric: true, format: F.d2 },
    { key: 'adot', label: 'aDOTfc', numeric: true, format: F.d1 },
    { key: 'fd', label: '1stD', numeric: true }, { key: 'fdpct', label: '1stD%', numeric: true, format: F.d1 },
    { key: 'third', label: '3rd%', numeric: true, format: F.d1 }, { key: 'rztd', label: 'RZTD%', numeric: true, format: F.d1 },
  ...
  return <Tile title="Every defense — packed metrics" subtitle={`${sliceLabel(slicers)} · honors all rail filters · scroll horizontally`} span={12}>
    <QueryState q={q} height={460}>{rows => <DataTable rows={rows} columns={cols} defaultSort={{ key: 'ypp', dir: 'asc' }} tight zebra />}</QueryState>
  </Tile>
}
